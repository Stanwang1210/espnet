...
fleurs_asr = load_dataset(
    "google/xtreme_s", f"fleurs.{ args.lang}", cache_dir=download_dir, trust_remote_code=True
)
...
def create_csv(split):
    ...
    df.to_csv(f"{download_dir}/{args.lang}/{name}.tsv", index=False, sep="\t")
