...
reconstruct_dumpdir=dump_${fs}
if [ "${feats_type}" = raw ]; then
    data_feats="${dumpdir}/raw_${data_combo_name}_${codec_choice}_$(echo "$codec_hf_model_tag" | tr '/' '_')"
    reconstruct_data_feats="${reconstruct_dumpdir}/raw_${data_combo_name}_${codec_choice}_$(echo "$codec_hf_model_tag" | tr '/' '_')"
else
    log "${help_message}"
...
token_listdir="${dumpdir}/token_list/${token_type}/${codec_choice}_$(echo "$codec_hf_model_tag" | tr '/' '_')"
...
if [ -z "${tts_stats_dir}" ]; then
    tts_stats_dir="${expdir}/tts_stats_${feats_type}_${codec_choice}_$(echo "$codec_hf_model_tag" | tr '/' '_')"
...
if [ -z "${tts_exp}" ]; then
    tts_exp="${expdir}/tts_${tag}_${codec_choice}_$(echo "$codec_hf_model_tag" | tr '/' '_')"
fi
...
if ! "${skip_data_prep}"; then
    ...
    if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
        ...
        # ====== Recreating "wav_resyn_ESPnet.scp" ======
        # Kaldi-wav_resyn_ESPnet.scp, which can describe the file path with unix-pipe, like "cat /some/path |",
        ...
        log "Stage 2: Format wav_resyn_ESPnet.scp: data/ -> ${data_feats}/"
        for dset in "${train_set}" "${valid_set}" ${test_sets}; do
            ...
            rm -f ${data_feats}${_suf}/${dset}/{segments,wav_resyn_ESPnet.scp,reco2file_and_channel}
            _opts=
            ...
            scripts/audio/format_wav_scp.sh --nj "${nj}" --cmd "${train_cmd}" \
                ...
                "data/${dset}/wav_resyn_ESPnet.scp" "${data_feats}${_suf}/${dset}"
    ...
    if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
		...
		spk_embed_tag=xvector
                ...
                log "Stage 3.1: Extract X-vector with Kaldi: data/ -> ${data_feats}/${spk_embed_tag} (Require Kaldi)"
                ...
                for dset in "${train_set}" "${valid_set}" ${test_sets}; do
                    ...
                    <"${data_feats}${_suf}/${dset}/wav_resyn_ESPnet.scp.bak" \
                        utils/filter_scp.pl "${data_feats}/${spk_embed_tag}/${dset}/${spk_embed_tag}.scp" \
                        >"${data_feats}${_suf}/${dset}/wav_resyn_ESPnet.scp"
                    utils/fix_data_dir.sh "${data_feats}${_suf}/${dset}"
                done
            else
                # Assume that others toolkits are python-based
                log "Stage 3.1: Extract speaker embedding: data/ -> ${data_feats}/${spk_embed_tag} using python toolkits"
                ...
                for dset in "${train_set}" "${valid_set}" ${test_sets}; do
                    ...
                    mkdir -p ${data_feats}/${spk_embed_tag}/${dset}
                    utils/fix_data_dir.sh ${data_feats}${_suf}/${dset}
                    utils/utt2spk_to_spk2utt.pl "${data_feats}${_suf}/${dset}/utt2spk" > "${data_feats}${_suf}/${dset}/spk2utt"
                    cp -r ${data_feats}${_suf}/${dset}/spk2utt ${data_feats}/${spk_embed_tag}/${dset}

                    ${_cmd} --gpu "${_ngpu}" ${data_feats}/${spk_embed_tag}/${dset}/spk_embed_extract.log \
    ...
    #         <"${data_feats}/org/${dset}/wav_resyn_ESPnet.scp" \
    #             utils/filter_scp.pl "${data_feats}/${dset}/utt2num_samples"  \
    #             >"${data_feats}/${dset}/wav_resyn_ESPnet.scp"
    ...
    #         if "${use_spk_embed}"; then
    #             cp "${data_feats}/${spk_embed_tag}/${dset}"/${spk_embed_tag}.{scp,scp.bak}
    #             <"${data_feats}/${spk_embed_tag}/${dset}/${spk_embed_tag}.scp.bak" \
    #                 utils/filter_scp.pl "${data_feats}/${dset}/wav_resyn_ESPnet.scp"  \
    #                 >"${data_feats}/${spk_embed_tag}/${dset}/${spk_embed_tag}.scp"
    #         fi
    #     done
    # fi
    ...
    if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
        log "Stage 5: Generate token_list from ${srctexts} to ${token_list}"
        ...
        cat ${data_feats}/${train_set}/text | awk ' { if( NF != 1 ) print $0; } ' >"${data_feats}/srctexts"
...
if ! "${skip_train}"; then
    if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
        ...
        if "${use_spk_embed}"; then
            _spk_embed_train_dir="${data_feats}/${spk_embed_tag}/${train_set}"
            _spk_embed_valid_dir="${data_feats}/${spk_embed_tag}/${valid_set}"
...
if ! "${skip_eval}"; then
    if [ ${stage} -le 8 ] && [ ${stop_stage} -ge 8 ]; then
        ...
        for dset in ${test_sets}; do
            ...
            # Add speaker embedding to the inputs if needed
            if "${use_spk_embed}"; then
                _spk_embed_dir="${data_feats}/${spk_embed_tag}/${dset}"
            ...
            if [ -e "${_logdir}/output.${_nj}/wav" ]; then
                ...
                done | LC_ALL=C sort -k1 > "${_dir}/wav/wav_resyn_ESPnet.scp"
            fi
    ...
    if [ ${stage} -le 9 ] && [ ${stop_stage} -ge 9 ]; then
        ...
        if "${use_spk_embed}"; then
            for dset in "${train_set}" ${test_sets}; do
                _opts+=" --option ${data_feats}/${spk_embed_tag}/${dset}/spk_${spk_embed_tag}.scp"
                _opts+=" --option ${data_feats}/${spk_embed_tag}/${dset}/spk_${spk_embed_tag}.ark"
            done
        fi
        if "${use_sid}"; then
