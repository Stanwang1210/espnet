...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    for lang in ${langs}; do
        ...
        log "Resampling WAV files to 16000 Hz" 
        log "corpus_dir: ${corpus_dir}/*/*.wav"
        # for wav_file in $(find ${corpus_dir} -name "*.wav"); do 
        #     ffmpeg -i "${wav_file}" -ar 16000 "${wav_file%.wav}_16000.wav" 
        #     mv "${wav_file%.wav}_16000.wav" "${wav_file}" 
        # done
